...
def find_creds(i):
    with open(i, newline='') as f:
        ...
        # Create and iterable dictreader object, extract file paths from it, and add those file paths to the file paths list
        file_event = csv.DictReader(f)
        for line in file_event:
            # users = line['username']
            file_paths.append(line['file_path'])
        
        # Iterate over the file_paths list, using regex to look for files that contain password-like words in them. Save each match to the password_files list.
